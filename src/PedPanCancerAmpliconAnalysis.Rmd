...
plot_gene_x_tumor_heatmap <- function(){
  gene_x_tumor_frequencytable   <- gene_x_tumor_amp_freq %>% 
    ...
    mutate(cancertype = fct_relevel(cancertype, c("CPG","EP","ETMR","HGG","MB","NBL","OS","PB","RB","RHB","other")))
...
tumor_amp_freq <- ecDNA_amplicons %>%
  ...
  summarize(count = n()) %>%
  mutate(cancer_type = fct_relevel(cancer_type,"CPG","EP","ETMR","HGG","MB","NBL","OS","PB","RB","RHB","other"))
...
x_barplot <- function(color=TRUE){
  ...
  bp.x <- ggplot(data = tumor_amp_freq, mapping = mapping) + 
    ...
    labs(x = "Cancer Type", y = "Patient tumors with frequently ecDNA-amplified oncogenes", tag="a")
  return(bp.x)
...
#TODOs
# Set height of top row independently of bottom row
