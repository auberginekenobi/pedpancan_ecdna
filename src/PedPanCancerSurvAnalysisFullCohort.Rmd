...
km_plot <- function(survObj,outfile=NULL){
  ...
  if (length(survObj$n) == 2){
    ...
    labels = c('cyclic','noncyclic','no fSCNA')
  }
  plt <- survObj %>% 
...
```{r}
data <- load_survival_data("../data/Supplementary Tables.xlsx") %>%
  # merge BFB into ecDNA since there aren't a lot
  mutate(amplicon_class = if_else(amplicon_class == "BFB", "ecDNA", amplicon_class))  %>% 
  # merge CNC into linear since there aren't a lot
  mutate(amplicon_class = if_else(amplicon_class == "Complex noncyclic", "Linear", amplicon_class))  %>% 
