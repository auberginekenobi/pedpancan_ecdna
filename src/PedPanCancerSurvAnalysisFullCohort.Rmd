...
km_plot <- function(survObj,outfile){
  ...
  survObj %>% 
   ggsurvfit(linewidth=0.2) +
   labs(x = 'Follow-up time (Months)',
        y = 'Overall Survival') +
   add_censor_mark(size = 1, alpha = 0.2) +
   add_confidence_interval() +
   add_risktable(risktable_stats = "n.risk", size=1.5, theme =      
                   theme_risktable_default(axis.text.y.size = 6,
                                    plot.title.size = 5)) +
   add_risktable_strata_symbol(size=4) + 

    #theme_classic(base_size=7, base_family="Arial",) 
    #theme(axis.text = element_text(size=7,colour="black"),
     #     plot.tag = element_text(size=7,face = "bold", colour = "black"))
  ...
  bmpName = paste(outfile, ".bmp", sep = "")
  ggsave(path="../out", filename=pdfName, device="pdf", width=2, height=2, units='in')
  ggsave(path="../out", device="png", filename=pngName, width=2, height=2, units='in')
  ggsave(path="../out", device="bmp", filename=bmpName, width=2, height=2, units='in')
...
m1
m1_unstrat <- coxph(Surv(OS_Months, OS_Status) ~ ecDNA + Cancer_Type_Abbrev, data = dd2)
m1_unstrat
coxZph <-cox.zph(m1_unstrat)
print(coxZph)
