...
import_hgg_data = function(path = "../manuscript/collaborators/Rishaan/pHGG/pHGG_Survival_Data_Merged.xlsx"){
    # Changelog: wrap in function, censor at 5y, drop HGNET, cleanup (drop unused transformations, drop_na), add ecDNA_status and amplified
    df <- read_excel(path)
    df <- df %>% 
        #rename_with(~ gsub("\\s+", "_", .x)) %>% # no need, already underscores
        rename_with(tolower) %>%
...
df %>% head
# (OSC removed redundant KM plots)
```
```{r}
